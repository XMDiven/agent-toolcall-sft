...
| 18 | 测试覆盖 | **421 passed**（无本地权重时为 419 passed + 2 skipped） | `uv run pytest -q` | `554238d` |
| 19 | 平台联动 | 平台既有测试 253 → **267 passed**；A/B 决策一致 9/12 | `docs/demo/router_ab.md` | `443d5e4` |
| 20 | 部署一致性 | Mac 与 CUDA **500/500 判定一致、498/500 逐字节一致** | `docs/evidence/deployment_parity.md` | `d57d5f4` |
...
冻结测试集中**纯 ASCII（英文）样本 0 条**，含 4 字母以上拉丁词仅 2 条，文本长度中位 25 字符、最长 **79 字符**，且为单轮设计。语料按设计只覆盖中文短文本客服场景。

ROADMAP 第 4 节的"中文、英文、Unicode、长输入和重复请求稳定处理"一项**如实留空未勾**——本项目没有做这项验证。

### ✗ "模型达到生产可用标准"

推理接口与平台联动已完成，但仍有五项拦着这个说法：**1.11% 的危险写误调用率**（不适用于无人工复核的自动退款）、延迟相对基线上升 84%、交付 Adapter 并非验证集最优点（`eval_v2.md` §8）、多语言与长输入未验证、服务无进程守护。

**正确说法：** 已具备可插拔的推理接口并完成与平台的 A/B 联动；距离生产可用仍有上述五项差距。
