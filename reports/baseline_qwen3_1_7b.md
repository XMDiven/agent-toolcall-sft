...
> **已作废，保留为历史证据。** 本文件基于 `template_version = v1` 与 `PROMPT_VERSION = v1`，两者均已被独立复核判定存在缺陷（跨 split 泄漏、manifest 未冻结标签、提示词非模型原生协议）。请改用 `reports/baseline_qwen3_1_7b_v2.md`，本文件的数字不得再作为基线或结论引用。
